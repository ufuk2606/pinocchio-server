...
router.post(
  "/mittagsmenu",
  upload.single("mittagsmenu"),
  async (req, res, next) => {
    try {
      const mittagsmenuPDF = req.file;
      const mittagsmenu = await mittagsmenüRepository.addPDF(
        mittagsmenuPDF.buffer
      );
...
router.post(
  "/speisekartenmenu",
  upload.single("speisekartenmenu"),
  async (req, res, next) => {
    try {
      const speisekartenmenüPDF = req.file;
      const speisekartenmenü = await speisekartenmenüRepository.addPDF(
        speisekartenmenüPDF.buffer
      );
...
router.put("/image", upload.single("profileImage"), async (req, res, next) => {
  try {
    ...
    const updatedImage = await userRepository.updateUserImage(
      user.id,
...
router.get("/altebestellungen", async (req, res, next) => {
  try {
    const userEmail = req.query.email;
    const user = await userRepository.getUserByEmail(userEmail);
    const alteBestellungen = await bestellungRepository.getAlteBestellung(
      user.id
    );
  ...
  async (req, res, next) => {
    try {
      ...
      for (let i = 0; i < galleryImages.length; i++) {
        const fileName = `pinocchio_${uuidv4()}`;
        const galleryImagePath = await s3Service.uploadFile(
          galleryImages[i].buffer,
          fileName,
          process.env.AWS_BUCKET_NAME
        );

        await galleryRepository.createImage(galleryImagePath.Location);
      }
